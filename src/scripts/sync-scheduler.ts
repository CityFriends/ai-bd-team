...
async function syncContracts(): Promise<number> {
  ...
  for (const page of pages) {
    ...
    const record = {
      ...
      contract_vehicle:
        getSelectProperty(props['Vehicle']) !== 'None' ? getSelectProperty(props['Vehicle']) : null,
      ...
      our_role: 'Prime',
      relevant_naics:
        getSelectProperty(props['NAICS Code']) !== 'N/A'
          ? [getSelectProperty(props['NAICS Code'])]
          : null,
      tags: [
...
async function syncTeamingPartners(): Promise<number> {
  ...
  for (const page of pages) {
    ...
    const record = {
      ...
      set_asides: sbaDesignations.filter((d) =>
...
async function syncLaborRates(): Promise<number> {
  ...
  for (const page of pages) {
    ...
    const currentRate =
      getNumberProperty(props['Year 3 Rate (Current)']) ||
