...
async function importContracts() {
  ...
  for (const page of pages) {
    ...
    const programManager = getTextProperty(props['Program Manager']);
...
async function importTeamingPartners() {
  ...
  for (const page of pages) {
    ...
    const linkedin = getUrlProperty(props['Company LinkedIn Page']);
    const uei = getTextProperty(props['UEI (SAM)']);
    const contractVehicles = getTextProperty(props['Contract Vehicles or GSA Schedules']);
    ...
    const record = {
      ...
      notes: additionalCapabilities || (clients.length > 0 ? `Clients: ${clients.join(', ')}` : null),
    };
...
async function importLaborRates() {
  ...
  for (const page of pages) {
    ...
    const year4 = getNumberProperty(props['Year 4 Rate ']);
    const year5 = getNumberProperty(props['Year 5 Rate']);
...
async function main() {
  ...
  // Show summary
  const supabase = getSupabase();
  const { data: ppCount } = await supabase.from('past_performance').select('id', { count: 'exact' });
  const { data: tpCount } = await supabase.from('teaming_partners').select('id', { count: 'exact' });
