...
const AGENCY_PATTERNS: Record<string, { code: string; name: string; keywords: string[] }> = {
  VA: {
    ...
    keywords: ['va', 'veterans', 'veteran affairs'],
  },
  HHS: {
    code: '075',
    name: 'Department of Health and Human Services',
    keywords: ['hhs', 'health human services', 'cms', 'cdc', 'nih', 'fda'],
  },
  DOD: {
    code: '097',
    name: 'Department of Defense',
    keywords: ['dod', 'defense', 'army', 'navy', 'air force', 'pentagon'],
  },
  DHS: {
    code: '070',
    name: 'Department of Homeland Security',
    keywords: ['dhs', 'homeland', 'tsa', 'fema', 'ice', 'cbp'],
  },
  DOJ: {
    code: '015',
    name: 'Department of Justice',
    keywords: ['doj', 'justice', 'fbi', 'dea', 'atf'],
  },
  ...
  STATE: {
    ...
    keywords: ['state department', 'state dept', 'usaid'],
  },
  GSA: {
    code: '047',
    name: 'General Services Administration',
    keywords: ['gsa', 'general services'],
  },
  SBA: {
    code: '073',
    name: 'Small Business Administration',
    keywords: ['sba', 'small business admin'],
  },
  EPA: {
    code: '068',
    name: 'Environmental Protection Agency',
    keywords: ['epa', 'environmental protection'],
  },
  ...
  DOT: {
    ...
    keywords: ['dot', 'transportation', 'faa', 'fhwa'],
  },
  ...
  OPM: {
    ...
    keywords: ['opm', 'personnel management'],
  },
};
...
function detectTopics(text: string): {
  ...
  const fpdsKeywords = [
    ...
    'piid',
    'idiq',
    // More conversational triggers for David's research
    'who is doing',
    'current contractor',
    'previous contract',
    'recompete',
  ...
  const needsCompetitorIntel =
    competitorName !== null || competitorKeywords.some((kw) => lowerText.includes(kw));

  // Detect GitHub URLs in the message
  const githubUrlMatch = text.match(/github\.com\/([^/\s]+)\/([^/\s>]+)/i);
  const githubUrl = githubUrlMatch
    ? `https://github.com/${githubUrlMatch[1]}/${githubUrlMatch[2].replace(/[>.,)]+$/, '')}`
    : null;
...
export async function gatherResearchContext(
  ...
  const shouldFetchNews =
    (agentName === 'maya' || agentName === 'david') && (topics.needsNews || topics.agency);

  const shouldFetchFPDS = agentName === 'david' && (topics.needsFPDS || topics.agency);

  const shouldFetchSpending = agentName === 'david' && (topics.needsSpending || topics.agency);

  const shouldFetchPartner =
    agentName === 'rosa' && (topics.needsPartnerCheck || topics.companyName);

  const shouldFetchFAR = (agentName === 'david' || agentName === 'james') && topics.needsFAR;
  ...
  const shouldFetchSAMOpportunities =
    agentName === 'maya' &&
      ...
      (async () => {
        try {
          ...
          if (topics.needsRiskNews) {
            console.log(
              `Research: Fetching risk/negative news for ${topics.agency!.name} from GovCon sources`
            );
            const riskTerms = [
              'cancel',
              'fraud',
            ...
            const matchedTerm =
              riskTerms.find((t) => topics.keywords.some((k) => k.includes(t))) ||
              'contract issues';
          ...
          if (topics.needsAwardNews) {
            console.log(
              `Research: Fetching contract award news for ${topics.agency!.name} from GovCon sources`
            );
        ...
        try {
          ...
          if (spendingResult.spending) {
            ...
            context.spending = {
              ...
              trend: trendSummary,
              source:
                spendingResult.source +
                ' (Note: USASpending shows AWARDED contracts with 2-4 week lag. For new opportunities, incumbent data comes from prior similar contracts.)',
            };
          }
        } catch (err) {
          ...
          const agencyForecasts = topics.agency
            ? forecasts.filter(
                (f) =>
                  f.agency
                    .toLowerCase()
                    .includes(topics.agency!.name.toLowerCase().split(' ')[0]) ||
      ...
      (async () => {
        try {
          ...
          if (result.opportunitiesData && result.opportunitiesData.length > 0) {
            ...
            console.log(
              `Research: Found ${result.opportunitiesData.length} opportunities from SAM.gov`
            );
          } else {
  ...
  const shouldFetchCompetitorIntel =
    (agentName === 'david' || agentName === 'rosa') &&
      ...
      (async () => {
        try {
          ...
          if (companyToResearch) {
            ...
            console.log(
              `Research: Fetching competitor intel for ${companyToResearch}${hasRecent ? ' (have recent)' : ''}`
            );
  ...
  const shouldFetchGitHubRepo =
    agentName === 'marcus' && (topics.needsGitHubRepo || topics.githubUrl);
      ...
      (async () => {
        try {
          ...
          if (analysis) {
            ...
            console.log(
              `Research: Analyzed ${analysis.owner}/${analysis.repo} - ${analysis.techStack.length} tech stack items detected`
            );
          } else {
